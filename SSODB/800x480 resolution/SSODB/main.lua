...
-- Rendered height of each font. The values here are the 480x320 defaults;
-- measureFonts() replaces them with what the radio actually reports, because
-- EdgeTX scales its fonts with screen size -- on an 800x480 radio they are
-- roughly half again as tall, and assuming the small-screen metrics is what
-- made text overflow its buttons there.
local FONT_H = { [SMLSIZE] = 13, [MIDSIZE] = 24, [DBLSIZE] = 32 }

local function measureFonts()
  if type(lcd) ~= "table" or type(lcd.sizeText) ~= "function" then return end
  for _, f in ipairs({ SMLSIZE, MIDSIZE, DBLSIZE }) do
    local ok, _, h = pcall(lcd.sizeText, "Ag", f)
    if ok and type(h) == "number" and h > 0 then FONT_H[f] = h end
  end
end

-- Vertical offset that centres `font` inside a box of height `h`.
-- Rendered width of a string. Used to place the battery-bar labels exactly
-- rather than from a per-character estimate, which differs per radio.
local function textWidth(str, font)
  if lcd and type(lcd.sizeText) == "function" then
    local ok, w = pcall(lcd.sizeText, tostring(str), font)
    if ok and type(w) == "number" and w > 0 then return w end
  end
  return #tostring(str) * math.max(6, math.floor((FONT_H[font] or 24) * 0.55))
end
...
-- ===========================================================================
-- SCREEN GEOMETRY AND LAYOUT PROFILES
-- ===========================================================================
-- The widget supports two transmitter resolutions. Rather than shipping two
-- copies of the file (which is what happened before, and the copies drifted
-- until the same bug had to be fixed in each), every position lives in a
-- profile and the right one is copied into the working tables at startup.
--
-- FLIGHT/ED/MI/KB stay module-level tables so the several hundred existing
-- references (FLIGHT.timerX and so on) keep working untouched; applyLayout()
-- fills them in. SCREEN_W/SCREEN_H are plain variables read at call time.
--
-- The FLIGHT numbers are not guesses: each was measured from the pixels of
-- the corresponding Background.png, because the cards are drawn in the
-- artwork and the widget has to line up with them.
local SCREEN_W, SCREEN_H = 480, 320
local FLIGHT, ED, MI, KB, PT, SEL, SUM = {}, {}, {}, {}, {}, {}, {}
-- Title-bar heights, derived from the measured fonts in applyLayout(). Screen
-- headers carry BIG text, modal headers MEDIUM; both used to be fixed at
-- 36-40px, which clipped the title on radios with larger glyphs.
local BAR_H, MODAL_H = 40, 36
-- Popup geometry, derived in applyLayout() from the screen size and the
-- measured fonts. These were fixed pixel sizes, which left the popups
-- marooned in the middle of a larger screen with their text overflowing.
local MD = {}
...
local PROFILES = {
  [480] = {
    ...
    FLIGHT = {
      ...
      -- This screen only has 117px between the row-2 cards (end 98) and the
      -- battery bar (starts 215), so it cannot take the 252x150 pictures the
      -- 800x480 layout uses; images are scaled to 192x114 here instead.
      modelX = 0, modelY = 101, modelW = 192, modelH = 114,
      -- Cards from Background.png: flight-time y 104..153, throttle y 160..209,
      -- both x 197..475. Each carries its own printed label along the bottom,
      -- so the bars sit in the free area above with the value inside.
      timerX = 197, timerY = 104, timerW = 278, timerH = 49,
      timerSweepX = 207, timerSweepY = 106, timerSweepW = 258, timerSweepH = 30,
      ...
      rateX = 215, profX = 257, topCardY = 45,
      ...
      imgY = 110,
      telem = {
        {key="volt",    x=15,  y=260}, {key="minCell", x=15,  y=290},
        {key="temp",    x=174, y=260}, {key="maxCurr", x=174, y=290},
        {key="bec",     x=333, y=260}, {key="rqly",    x=333, y=290},
      },
    },
    ED = {
      headH  = 36,
      lRowX  = 10,  lRowW = 414, lRowH = 38, lRowGap = 4, lVisible = 5, lTop = 42,
      lSbarX = 432, lSbarW = 38, lAddY = 256, lAddH = 44,
      ...
      listX2 = 8,   listW2 = 96,
      saveX  = 386, saveW = 86,
      hdrTxtX = 245, hdrTxtY = 8, emptyY = 120,
      toastX = 140, toastY = 6, toastTextY = 9, toastW = 130, toastH = 24,
    },
    MI = {
      titleX = 90, titleY = 4,
      infoX = 15, modelY = 48, imageY = 70,
      emptyY1 = 150, emptyY2 = 172,
      listX = 10,  listW = 250, rowH = 30, rowGap = 4, visible = 5, contentY = 88,
      prevX = 280, prevY = 88,
      btnY  = 268, btnW = 110, btnH = 42,
      backX = 185, confirmX = 110, cancelX = 260,
    },
    -- PID tuning screen. It draws its own background, so these are chosen
    -- rather than measured.
    SEL = {
      titleX = 111, titleY = 4,
      noticeX = 240, noticeY = 300,
      syncX = 470, syncY = 4,
      loadX = 27,  loadY = 65,
      colLX = 15,  colRX = 245, cardW = 220, cardH = 58,
      gridY = 50,  rowStep = 70,
      btnY = 268,  btnW = 110, btnH = 42,
      backX = 110, editX = 260,
      popW = 360,  popH = 200,
    ...
    SUM = {
      titleX = 14, titleY = 4,
      colLX = 10, colRX = 245, cardW = 225, cardH = 42,
      gridY = 48, rowStep = 48,
      labelDX = 12, labelDY = 3, valueDY = 17, edgeW = 4,
      btnX = 185, btnY = 268, btnW = 110, btnH = 42,
    },
    PT = {
    ...
    KB = {
      titleX = 12, titleY = 5,
      boxX = 10, boxY = 44, boxW = 460, boxH = 36, boxTextX = 20, boxTextY = 48,
      keyTop = 88, keyH = 34, keyGap = 4, keyW10 = 44, keyW9 = 48,
      actY = 256, actH = 54, actX = 8, actGap = 6,
      numBoxH = 40, numRangeX = 468, numRangeY = 10, numUnitY = 54,
      padX = 14, padY = 90, numKeyW = 90, numKeyH = 38, numGap = 5,
      colX = 310, colW = 156,
      numActY = 264, numActH = 46, numActW = 220,
    },
  },

  [800] = {
    w = 800, h = 480,
    -- Measured from the 800x480 Background.png:
    --   header bar        y 0..55          model-name card  x 6..280,  y 106..145
    --   Rate              x 291..352       Profile          x 362..423
    --   VCell             x 433..536       Curr             x 546..649
    --   RQly              x 659..792       all top cards    y 63..146
    --   flight-time card  x 291..792, y 156..228, printed label from y 206
    --   throttle card     x 291..792, y 239..311, printed label from y 288
    --   model image area  x 0..286,  y 150..383
    --   clock box         x 550..704, y 5..52
    --   bottom row 1      y 388..423       bottom row 2     y 433..469
    --   bottom cards      x 7..259, 273..526, 538..791
    FLIGHT = {
      modelTitleX = 6, modelTitleY = 4,
      clockX = 745, clockY = 18,
      -- Model-name card is y 106..145; the top cards are y 63..146.
      row1Y = 63, row2Y = 106, rowH = 40, cardW = 274,
      cardXL = 6, cardXR = 6,
      txBarX = 618, txBarY = 16, txBarW = 70, txBarH = 24,
      txTextX = 560, txTextY = 18,
      -- Model pictures are 252x150 here. The artwork's image area is
      -- x 0..286, y 150..383, so at 8,158 the picture occupies 8..260 and
      -- 158..308, clearing both the area edge and the battery bar at 320.
      -- Images that aren't this size are scaled to it when loaded.
      modelX = 8, modelY = 158, modelW = 252, modelH = 150,
      timerX = 291, timerY = 156, timerW = 501, timerH = 72,
      timerSweepX = 301, timerSweepY = 158, timerSweepW = 481, timerSweepH = 44,
      thrX = 291, thrY = 239, thrW = 501, thrH = 72,
      thrBarX = 301, thrBarY = 241, thrBarW = 481, thrBarH = 43,
      -- Nothing is printed between the image area (ends 383) and the bottom
      -- rows (start 388), so the battery bar gets its own band above them.
      batteryX = 8, batteryY = 320, batteryW = 784, batteryH = 58,
      rateX = 321, profX = 392, topCardY = 70,
      minCellX = 484, minCellY = 78,
      maxCurrX = 597, maxCurrY = 78,
      rqlyIconX = 668, rqlyIconY = 78,
      rqlyTextX = 726, rqlyTextY = 80,
      imgY = 158,
      telem = {
        {key="volt",    x=30,  y=396}, {key="minCell", x=30,  y=441},
        {key="temp",    x=296, y=396}, {key="maxCurr", x=296, y=441},
        {key="bec",     x=561, y=396}, {key="rqly",    x=561, y=441},
      },
    },
    ED = {
      headH  = 44,
      lRowX  = 14,  lRowW = 700, lRowH = 52, lRowGap = 6, lVisible = 6, lTop = 54,
      lSbarX = 726, lSbarW = 58, lAddY = 412, lAddH = 56,
      pRowX  = 14,  pRowW = 772, pRowH = 56, pRowGap = 7, pTop = 56,
      hBtnY  = 6,   hBtnH = 32,
      titleX = 18,  titleY = 5,
      backX  = 660, backW = 124,
      listX2 = 14,  listW2 = 130,
      saveX  = 654, saveW = 130,
      hdrTxtX = 400, hdrTxtY = 10, emptyY = 190,
      toastX = 300, toastY = 8, toastTextY = 12, toastW = 180, toastH = 28,
    },
    MI = {
      titleX = 240, titleY = 6,
      infoX = 20, modelY = 62, imageY = 92,
      emptyY1 = 210, emptyY2 = 240,
      listX = 16,  listW = 420, rowH = 42, rowGap = 6, visible = 6, contentY = 120,
      prevX = 460, prevY = 120,
      btnY  = 414, btnW = 150, btnH = 52,
      backX = 325, confirmX = 200, cancelX = 450,
    },
    SEL = {
      titleX = 250, titleY = 6,
      noticeX = 400, noticeY = 452,
      syncX = 784, syncY = 8,
      loadX = 40,  loadY = 100,
      colLX = 20,  colRX = 408, cardW = 372, cardH = 76,
      gridY = 66,  rowStep = 90,
      btnY = 404,  btnW = 160, btnH = 56,
      backX = 190, editX = 450,
      popW = 520,  popH = 260,
    },
    SUM = {
      titleX = 18, titleY = 6,
      colLX = 16, colRX = 408, cardW = 376, cardH = 68,
      gridY = 66, rowStep = 76,
      labelDX = 16, labelDY = 4, valueDY = 26, edgeW = 5,
      btnX = 320, btnY = 404, btnW = 160, btnH = 56,
    },
    PT = {
      titleX = 16, titleY = 10, profX = 540, profY = 15,
      backX = 668, backY = 6, backW = 116, backH = 34,
      bodyY = 46, headH = 46,
      tabY = 52, tabH = 36, tabW = 140, tabX1 = 16, tabX2 = 164, tabX3 = 312,
      statusX = 470, msgX = 30, msgY1 = 140, msgY2 = 166, msgY3 = 192,
      -- Column widths chosen so the grids span the wider screen instead of
      -- bunching into the left two-thirds. A PID cell is 106px wide
      -- (label + stepper + value + stepper), a gov cell 85px.
      govColW = 300, govStartX = 24, govStartY = 104,
      pidColW = 216, pidStartX = 2, pidStartY = 104,
      saveX = 30, saveY = 412, saveW = 170, saveH = 46,
    },
    KB = {
      titleX = 18, titleY = 8,
      boxX = 14, boxY = 60, boxW = 772, boxH = 48, boxTextX = 28, boxTextY = 66,
      keyTop = 126, keyH = 50, keyGap = 6, keyW10 = 74, keyW9 = 80,
      actY = 396, actH = 68, actX = 12, actGap = 10,
      numBoxH = 54, numRangeX = 782, numRangeY = 14, numUnitY = 74,
      padX = 24, padY = 130, numKeyW = 150, numKeyH = 56, numGap = 8,
      colX = 520, colW = 260,
      numActY = 398, numActH = 62, numActW = 370,
    },
  },
}

-- Copies the profile matching the zone into the working tables. Called from
-- create(), which is the first point the zone size is known.
local function applyLayout(zw, zh)
  measureFonts()
  local key = (safeNum(zw, 0) >= 800 and safeNum(zh, 0) >= 480) and 800 or 480
  local p = PROFILES[key]
  SCREEN_W, SCREEN_H = p.w, p.h
  for _, pair in ipairs({ {FLIGHT, p.FLIGHT}, {ED, p.ED}, {MI, p.MI}, {KB, p.KB}, {PT, p.PT},
                            {SEL, p.SEL}, {SUM, p.SUM} }) do
    local dst, src = pair[1], pair[2]
    for k in pairs(dst) do dst[k] = nil end
    for k, v in pairs(src) do dst[k] = v end
  end

  -- Fit the layout to the fonts the radio actually renders.
  --
  -- The profile numbers assume particular glyph sizes, but EdgeTX draws much
  -- larger text on an 800x480 radio (MEDIUM measures about 45px there against
  -- 24px on a 480x320 one). Growing the boxes alone is not enough: anything
  -- positioned below a box that grew has to move too, or rows end up running
  -- over the buttons beneath them. So heights are raised first, then every
  -- dependent position is recomputed from the screen edges inwards, and the
  -- number of visible rows is derived from the space that is actually left.
  local med = FONT_H[MIDSIZE] or 24
  local sml = FONT_H[SMLSIZE] or 13
  local big = FONT_H[DBLSIZE] or 32
  local function atLeast(t, k, need) if t[k] and t[k] < need then t[k] = need end end
  local function fitRows(top, bottom, rowH, gap)
    return math.max(1, math.floor((bottom - 8 - top + gap) / (rowH + gap)))
  end

  -- Title bars: a full-width screen header carries BIG text, a modal header
  -- MEDIUM. Both were fixed at 36-40px, which clipped the title outright.
  BAR_H   = big + 10
  MODAL_H = med + 10

  -- ---- editor: list screen ----
  atLeast(ED, "headH",  BAR_H)
  atLeast(ED, "hBtnH",  sml + 12)
  atLeast(ED, "lRowH",  med + sml + 6)
  atLeast(ED, "lAddH",  med + 14)
  atLeast(ED, "toastH", sml + 10)
  ED.hBtnY   = math.max(4, math.floor((ED.headH - ED.hBtnH) / 2))
  ED.titleY  = math.max(2, math.floor((ED.headH - big) / 2))
  ED.toastY  = math.max(2, math.floor((ED.headH - ED.toastH) / 2))
  ED.hdrTxtY = math.max(2, math.floor((ED.headH - med) / 2))
  ED.lTop    = ED.headH + 6
  ED.lAddY   = SCREEN_H - ED.lAddH - 10
  ED.lVisible= fitRows(ED.lTop, ED.lAddY, ED.lRowH, ED.lRowGap)

  -- ---- editor: parameter screen (six rows, all must fit) ----
  atLeast(ED, "pRowH", med + 14)
  ED.pTop = ED.headH + 8
  local pAvail = SCREEN_H - ED.pTop - 8
  if 6 * ED.pRowH + 5 * ED.pRowGap > pAvail then
    ED.pRowGap = 4
    ED.pRowH   = math.max(med + 4, math.floor((pAvail - 5 * ED.pRowGap) / 6))
  end

  -- ---- battery select ----
  atLeast(SEL, "cardH", med + sml + 10)
  atLeast(SEL, "btnH",  med + 14)
  SEL.titleY = math.max(2, math.floor((BAR_H - big) / 2))
  SEL.btnY   = SCREEN_H - SEL.btnH - 12
  SEL.gridY  = BAR_H + 10
  SEL.loadY  = SEL.gridY + 10
  -- three rows of cards have to sit between the header and the buttons
  local selAvail = SEL.btnY - 10 - SEL.gridY
  if 3 * SEL.cardH + 2 * 12 > selAvail then
    SEL.cardH = math.max(med + 6, math.floor((selAvail - 24) / 3))
  end
  SEL.rowStep = SEL.cardH + 12
  -- Two stacked lines inside the card: name over "(n flt)". These were fixed
  -- at +8 and +32, which assumed a 24px MEDIUM -- at 45px the name ran
  -- straight through the flight count underneath it.
  SEL.nameDY = math.max(2, math.floor((SEL.cardH - med - sml - 2) / 2))
  SEL.fltDY  = SEL.nameDY + med + 2
  SEL.noticeY = SCREEN_H - sml - 4
  atLeast(SEL, "popH", MODAL_H + big + med + SEL.btnH + 40)

  -- ---- summary ----
  atLeast(SUM, "cardH", sml + med + 8)
  SUM.titleY  = math.max(2, math.floor((BAR_H - big) / 2))
  SUM.gridY   = BAR_H + 8
  SUM.btnY    = SCREEN_H - SUM.btnH - 10
  -- Seven cards in two columns is four rows; they must clear the Back button.
  local sumAvail = SUM.btnY - 10 - SUM.gridY
  if 4 * SUM.cardH + 3 * 6 > sumAvail then
    SUM.cardH = math.max(sml + 4, math.floor((sumAvail - 18) / 4))
  end
  SUM.rowStep = SUM.cardH + 6
  SUM.valueDY = sml + 4

  -- ---- model image picker ----
  atLeast(MI, "rowH", med + 10)
  atLeast(MI, "btnH", med + 14)
  MI.titleY   = math.max(2, math.floor((BAR_H - big) / 2))
  MI.modelY   = BAR_H + 6
  MI.imageY   = MI.modelY + med + 2
  MI.contentY = MI.imageY + sml + 10
  MI.prevY    = MI.contentY
  MI.btnY     = SCREEN_H - MI.btnH - 10
  MI.visible  = fitRows(MI.contentY, MI.btnY, MI.rowH, MI.rowGap)
  MI.emptyY1  = MI.contentY + 40
  MI.emptyY2  = MI.emptyY1 + med + 4

  -- ---- text keyboard ----
  atLeast(KB, "keyH", sml + 14)
  atLeast(KB, "actH", med + 16)
  atLeast(KB, "boxH", med + 10)
  KB.titleY   = math.max(2, math.floor((BAR_H - big) / 2))
  KB.boxY     = BAR_H + 6
  KB.boxTextY = KB.boxY + math.max(2, math.floor((KB.boxH - med) / 2))
  KB.actY     = SCREEN_H - KB.actH - 10
  KB.keyTop   = KB.boxY + KB.boxH + 10
  local kbAvail = KB.actY - 10 - KB.keyTop
  if 4 * KB.keyH + 3 * KB.keyGap > kbAvail then
    KB.keyH = math.max(sml + 6, math.floor((kbAvail - 3 * KB.keyGap) / 4))
  end

  -- ---- numeric pad ----
  atLeast(KB, "numKeyH", med + 10)
  atLeast(KB, "numActH", med + 16)
  atLeast(KB, "numBoxH", big + 10)
  KB.numRangeY = math.max(2, math.floor((BAR_H - sml) / 2))
  KB.numActY   = SCREEN_H - KB.numActH - 10
  KB.padY      = BAR_H + 6 + KB.numBoxH + 10
  KB.numUnitY  = BAR_H + 6 + math.max(2, math.floor((KB.numBoxH - med) / 2))
  local padAvail = KB.numActY - 8 - KB.padY
  if 4 * KB.numKeyH + 3 * KB.numGap > padAvail then
    KB.numKeyH = math.max(med + 2, math.floor((padAvail - 3 * KB.numGap) / 4))
  end

  -- ---- popups ----
  -- Sized as a fraction of the screen rather than in fixed pixels, so they
  -- keep their proportions at any resolution; the fractions are the ratios
  -- the original 480x320 sizes had. Internals come from the font metrics.
  MD.pickW    = math.floor(SCREEN_W * 0.92)
  MD.pickH    = math.floor(SCREEN_H * 0.88)
  MD.slotW    = math.floor(SCREEN_W * 0.71)
  MD.slotH    = math.floor(SCREEN_H * 0.69)
  MD.modelsW  = math.floor(SCREEN_W * 0.92)
  MD.modelsH  = math.floor(SCREEN_H * 0.84)
  MD.confW    = math.floor(SCREEN_W * 0.79)
  MD.confH    = math.floor(SCREEN_H * 0.66)
  MD.itemH    = math.max(34, sml + 16)          -- a list row inside a popup
  MD.btnH     = math.max(38, med + 14)          -- a popup action button
  MD.listTop  = MODAL_H + 9                     -- first row, below the title bar
  -- Rows that fit between the title bar and the action row.
  MD.visible  = math.max(1, math.floor((MD.modelsH - MD.listTop - MD.btnH - 18) / (MD.itemH + 4)))
  MD.btnDY    = MD.modelsH - MD.btnH - 10       -- action row, from the popup top
  MD.pickBtnDY= MD.pickH   - MD.btnH - 10
  MD.pickVis  = math.max(1, math.floor((MD.pickH - MD.listTop - MD.btnH - 18) / (MD.itemH + 4)))
  -- First line of body text, clear of the title bar, and bottom-anchored
  -- action rows for the popups that aren't list-based.
  MD.bodyTop   = MODAL_H + 12
  MD.confBtnDY = MD.confH - MD.btnH - 14
  MD.slotBtnH  = math.max(32, sml + 12)
  MD.slotBtnDY = MD.slotH - MD.slotBtnH - 12
  MD.slotKeyH  = math.max(40, med + 8)
  MD.slotTop   = MD.bodyTop + sml + 10
  -- Stacked body lines for the confirmation popups (title, detail, note).
  MD.line2     = MD.bodyTop + big + 6
  MD.line3     = MD.bodyTop + big + med + 10
  MD.emptyY    = MD.bodyTop + 30
  MD.emptyY2   = MD.bodyTop + 30 + sml + 6

  -- ---- PID tuning ----
  atLeast(PT, "headH", BAR_H)
  atLeast(PT, "tabH",  sml + 12)
  atLeast(PT, "backH", sml + 12)
  atLeast(PT, "saveH", sml + 12)
  PT.titleY = math.max(2, math.floor((PT.headH - med) / 2))
  PT.backY  = math.max(2, math.floor((PT.headH - PT.backH) / 2))
  PT.profY  = math.max(2, math.floor((PT.headH - sml) / 2))
  PT.bodyY  = PT.headH
  PT.tabY   = PT.headH + 4
  PT.pidStartY = PT.tabY + PT.tabH + 8
  PT.govStartY = PT.pidStartY
  PT.saveY  = SCREEN_H - PT.saveH - 12
...
-- Grouped into one table rather than 15 separate locals: Lua allows only 200
-- locals in a chunk and the layout tables need the room.
local STR = {
  SELECT_BATTERY     = "SELECT BATTERY",
  SUMMARY            = "POST-FLIGHT SUMMARY",
  EDIT               = "EDIT BATTERIES",
  CONFIRM            = "Confirm",
  BACK               = "Back",
  NO_IMAGE           = "[No Image]",
...
-- Opens a model picture and scales it to the size this layout draws them at
-- (FLIGHT.modelW x FLIGHT.modelH, which differs per resolution).
local function openModelBitmap(path)
  local bmp = safeOpenBitmap(path)
  if not bmp then return nil end
  return scaleBitmap(bmp, FLIGHT.modelW, FLIGHT.modelH)
end
...
-- Text keyboard and numeric pad. Both are full-screen modals with a title
-- bar, a value box, a key grid and an action row.
...
local function drawNumpadModal(widget, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  roundRect(zx, zy, SCREEN_W, SCREEN_H, 0, C_SURFACE)
  roundRect(zx, zy, SCREEN_W, BAR_H, 0, C_BLACK)
  lcdDrawText(zx+KB.titleX, zy+KB.titleY, spec.title, BIG+C_WHITE)
  lcdDrawText(zx+KB.numRangeX, zy+KB.numRangeY, spec.min .. " - " .. spec.max .. " " .. spec.unit, SMALL+RIGHT+C_LABEL)
  ...
  roundRect(zx+KB.boxX, zy+KB.boxY, KB.boxW, KB.numBoxH, 4, C_CARD)
  lcdDrawText(zx+KB.boxTextX, zy+KB.boxY+6, (buf == "" and "0" or buf) .. "_", BIG + valColor)
  if spec.unit ~= "" then
    lcdDrawText(zx+KB.boxX+KB.boxW, zy+KB.numUnitY, spec.unit, MEDIUM+RIGHT+C_LABEL)
  end
  ...
  local keyW, keyH, gap = KB.numKeyW, KB.numKeyH, KB.numGap
  local padX, padY = zx + KB.padX, zy + KB.padY
  ...
  local rX, rW = zx + KB.colX, KB.colW
  ...
  -- Bottom actions.
  local aY, aH = zy + KB.numActY, KB.numActH
  if drawButton(widget, tx, ty, clickEvent, zx + KB.padX, aY, KB.numActW, aH, "Done", {color=C_TEAL, font=MEDIUM,slop=2, focus = (kbFocus == OK_F)}) then
    commitEditField(widget, field, widget.keyboardBuffer)
    widget.editingField = nil
    clickEvent = false
  end
  if drawButton(widget, tx, ty, clickEvent, zx + KB.padX + KB.numActW + 12, aY, KB.numActW, aH, "Cancel", {color=C_GREY, font=MEDIUM,slop=2}) then
    widget.editingField = nil
    clickEvent = false
  end
...
local function drawKeyboardModal(widget, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  roundRect(zx, zy, SCREEN_W, SCREEN_H, 0, C_SURFACE)
  roundRect(zx, zy, SCREEN_W, BAR_H, 0, C_BLACK)
  lcdDrawText(zx+KB.titleX, zy+KB.titleY, title, BIG+C_WHITE)

  roundRect(zx+KB.boxX, zy+KB.boxY, KB.boxW, KB.boxH, 4, C_CARD)
  lcdDrawText(zx+KB.boxTextX, zy+KB.boxTextY, (widget.keyboardBuffer or "") .. "_", MEDIUM+C_ROW_TXT)

  local startY = zy + KB.keyTop
  local keyH, gap = KB.keyH, KB.keyGap
  ...
  for rIdx, row in ipairs(KEYBOARD_ROWS) do
    local keyW = (#row == 10) and KB.keyW10 or KB.keyW9
    local rowW = #row * keyW + (#row - 1) * gap
    local startX = zx + math.floor((SCREEN_W - rowW) / 2)
  ...
  local actY = zy + KB.actY
  local actH = KB.actH

  local bsX, bsW = zx + KB.actX, 84
...
local function drawModelsModal(widget, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  if widget.modelPicker then
    local popW, popH = MD.pickW, MD.pickH
    -- Focus: 1..#known = model rows, then Type name, then Done.
    local pickFocus = safeNum(widget.modelPickerFocus, 1)
    local popX = zx + math.floor((SCREEN_W - popW) / 2)
    local popY = zy + math.floor((SCREEN_H - popH) / 2)

    roundRect(popX - 2, popY - 2, popW + 4, popH + 4, 8, C_TEAL)
    roundRect(popX, popY, popW, popH, 6, C_SURFACE)
    roundRect(popX, popY, popW, MODAL_H, 6, C_BLACK)
    lcdDrawText(popX + popW/2, popY + textCenterY(MODAL_H, MEDIUM), "ASSIGN MODELS", MEDIUM+CENTER+C_WHITE)
    ...
    local VISIBLE = MD.pickVis
    local itemH   = MD.itemH
    ...
    if #known == 0 then
      lcdDrawText(popX + popW/2, popY + MD.emptyY, "No models known yet", SMALL+CENTER+C_LABEL)
      lcdDrawText(popX + popW/2, popY + MD.emptyY2, "Connect a model, or type a name", SMALL+CENTER+C_LABEL)
    else
      local listY = popY + MD.listTop
      for vi = 1, VISIBLE do
        local i = vi + scroll
        local mName = known[i]
      ...
      if scroll > 0 then
        if drawButton(widget, tx, ty, clickEvent, sbX, popY + MD.listTop, sbW, MD.itemH, "^", {color=C_BLUE}) then
          scroll = scroll - 1
          widget.modelPickerScroll = scroll
      ...
      if scroll < maxScroll then
        local dY = popY + MD.listTop + (VISIBLE - 1) * (itemH + 4)
    ...
    -- Bottom-anchored: this was popY + 230, which left the buttons stranded
    -- half-way up once the popup grew with the screen.
    local pbY, pbH = popY + MD.pickBtnDY, MD.btnH
  ...
  if widget.slotPickerModel then
    ...
    local popW, popH = MD.slotW, MD.slotH
    local popX = zx + math.floor((SCREEN_W - popW) / 2)
    local popY = zy + math.floor((SCREEN_H - popH) / 2)
    ...
    lcdDrawText(popX + popW/2, popY + textCenterY(MODAL_H, SMALL), titleStr, SMALL+CENTER+C_WHITE)

    lcdDrawText(popX + popW/2, popY + MD.bodyTop, "Select Battery Number (1-6):", SMALL+CENTER+C_LABEL)
    ...
    local btnW, btnH = math.floor((popW - 80) / 3), MD.slotKeyH
    local startX = popX + 25
    local startY = popY + MD.slotTop
    ...
    local canX, canY, canW, canH = popX + math.floor((popW - 100) / 2), popY + MD.slotBtnDY, 100, MD.slotBtnH
  ...
  local popW, popH = MD.modelsW, MD.modelsH
  local popX = zx + math.floor((SCREEN_W - popW) / 2)
  local popY = zy + math.floor((SCREEN_H - popH) / 2)
  ...
  lcdDrawText(popX + 15, popY + textCenterY(MODAL_H, MEDIUM), headTitle, MEDIUM+C_WHITE)
  ...
  local listY = popY + MD.listTop
  local itemH = MD.itemH
  local shown = math.min(MD.visible, #modelList)
  ...
  if #modelList == 0 then
    lcdDrawText(popX + popW/2, popY + MD.emptyY, "No models assigned (Global Battery)", SMALL+CENTER+C_LABEL)
  else
    for i = 1, math.min(MD.visible, #modelList) do
...
local function saveEditProfiles(widget)
  ...
  if ok then
    ...
    widget.editStatusMsg = STR.SAVED
    ...
    widget.editStatusMsg = STR.SAVE_ERR
  end
...
local function drawEditToast(widget, zx, zy)
  local err = (widget.editStatusMsg == STR.SAVE_ERR)
  local tW, tH = ED.toastW, ED.toastH
  local tX = zx + ED.toastX
  roundRect(tX, zy + ED.toastY, tW, tH, 4, err and C_RED or C_TEAL)
  lcdDrawText(tX + tW/2, zy + ED.toastTextY, widget.editStatusMsg, SMALL+CENTER+C_WHITE)
end
...
local function drawEditListScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  roundRect(zx, zy, SCREEN_W, ED.headH, 0, C_BLACK)
  if editToastActive(widget) then
    drawEditToast(widget, zx, zy)
  else
    lcdDrawText(zx + ED.titleX, zy + ED.titleY, STR.EDIT, BIG + C_WHITE)
  end
  ...
  if drawButton(widget, tx, ty, bodyClick, zx+ED.backX, zy+ED.hBtnY, ED.backW, ED.hBtnH, "Back",
  ...
  if #eps == 0 then
    -- math.floor: SCREEN_W/2 is a float in Lua 5.3, and the literal it
    -- replaced was an integer.
    lcdDrawText(zx + math.floor(SCREEN_W/2), zy + ED.emptyY, "No batteries yet", MEDIUM+CENTER+C_LABEL)
  end
  ...
  if hasModal then
    local b = eps[delIdx]
    local popW, popH = MD.confW, MD.confH
    local popX = zx + math.floor((SCREEN_W - popW) / 2)
    local popY = zy + math.floor((SCREEN_H - popH) / 2)

    roundRect(popX - 2, popY - 2, popW + 4, popH + 4, 8, C_RED)
    roundRect(popX, popY, popW, popH, 6, C_SURFACE)
    roundRect(popX, popY, popW, MODAL_H, 6, C_RED)
    lcdDrawText(popX + popW/2, popY + textCenterY(MODAL_H, MEDIUM), "DELETE BATTERY?", MEDIUM+CENTER+C_WHITE)

    local pName = b.name or "Battery"
    if #pName > 22 then pName = string.sub(pName, 1, 20) .. "~" end
    lcdDrawText(popX + popW/2, popY + MD.bodyTop, pName, BIG+CENTER+C_ROW_TXT)

    local info = safeNum(b.cap, 0) .. " mAh  |  " .. safeNum(b.flights, 0) .. " flights"
    lcdDrawText(popX + popW/2, popY + MD.line2, info, MEDIUM+CENTER+C_LABEL)
    lcdDrawText(popX + popW/2, popY + MD.line3, "This cannot be undone.", SMALL+CENTER+C_LABEL)
...
local function drawEditBattScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  roundRect(zx, zy, SCREEN_W, ED.headH, 0, C_BLACK)
  if drawButton(widget, tx, ty, clickEvent, zx+ED.listX2, zy+ED.hBtnY, ED.listW2, ED.hBtnH, "< List",
  ...
  if editToastActive(widget) then
    ...
    lcdDrawText(zx + ED.hdrTxtX, zy + ED.hdrTxtY, hdr, MEDIUM + CENTER + C_WHITE)
  end

  if drawButton(widget, tx, ty, clickEvent, zx+ED.saveX, zy+ED.hBtnY, ED.saveW, ED.hBtnH, "Save",
...
local function drawFlightScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  if widget.bgBmp then
    ...
    roundRect(zx, zy, SCREEN_W, BAR_H, 5, C_NAVY)
  end
  ...
  if mode ~= v.mode or rpm ~= v.rpm then
    ...
    if mode < -512 then
      s.eTxt, s.eCol = STR.ENGINE_OFF, C_BLACK
    elseif mode < 512 then
      s.eTxt, s.eCol = STR.IDLE, C_BLACK
    elseif safeNum(rpm, 0) >= RPM_RUNNING then
      ...
      s.eTxt, s.eCol = STR.ENGINE_ON, C_HV
    end
  end

  -- Armed/Disarmed
  roundRect(zx + FLIGHT.cardXL, zy + FLIGHT.row1Y, FLIGHT.cardW, FLIGHT.rowH, 4, isArmed and C_RED or C_GREEN)
  lcdDrawText(zx + FLIGHT.cardXL+FLIGHT.cardW/2, zy + FLIGHT.row1Y+2, isArmed and STR.ARMED or STR.DISARMED, MEDIUM+CENTER+C_WHITE)
  ...
  elseif not widget.bgBmp then
    ...
    lcdDrawText(zx + FLIGHT.modelX+92, zy + FLIGHT.modelY+42, STR.NO_IMAGE, SMALL+CENTER+C_WHITE)
  end
  ...
  -- Labels are anchored to the bar's own edges rather than at fixed offsets
  -- from its left side. The old +260 / +458 were sized for a 468px bar; on
  -- the 784px bar they left the percentage and the capacity bunched together
  -- in the middle with a third of the bar empty to their right.
  --
  --   name  -> left edge
  --   %     -> centred in the bar
  --   mAh   -> right edge
  local battTextY = barY + textCenterY(barH, MEDIUM)
  local usedStr   = s.battUsed or "0mAh"
  local pctStr    = s.battPct  or "100%"

  -- The name is truncated to the room left of the centred percentage rather
  -- than to a fixed 16 characters: 16 fits comfortably on the wide bar but
  -- would just touch the percentage on the narrow one.
  -- Names get up to 20 characters. They are shortened further only if the
  -- measured text genuinely will not fit beside the other two labels.
  if #displayName > 20 then displayName = string.sub(displayName, 1, 19) .. "~" end

  -- Widths are measured, not estimated, and only re-measured when the string
  -- changes. The percentage sits at the middle of the bar, but slides right
  -- if a long name would otherwise run into it -- so a 20-character name and
  -- the capacity both keep their space.
  if displayName ~= v.nameStr then
    v.nameStr, v.nameW = displayName, textWidth(displayName, MEDIUM)
  end
  if usedStr ~= v.usedStr then
    v.usedStr, v.usedW = usedStr, textWidth(usedStr, MEDIUM + BOLD)
  end
  if pctStr ~= v.pctStr then
    v.pctStr, v.pctW = pctStr, textWidth(pctStr, MEDIUM + BOLD)
  end

  local nameX  = barX + 12
  local usedX  = barX + barW - 12
  local gap    = math.max(12, math.floor(barH / 4))
  local pctW   = safeNum(v.pctW, 40)
  local leftLimit  = nameX + safeNum(v.nameW, 0) + gap + math.floor(pctW / 2)
  local rightLimit = usedX - safeNum(v.usedW, 0) - gap - math.floor(pctW / 2)
  local pctCentre  = barX + math.floor(barW / 2)
  if pctCentre < leftLimit  then pctCentre = leftLimit  end
  if pctCentre > rightLimit then pctCentre = rightLimit end

  lcdDrawText(nameX,     battTextY, displayName, MEDIUM+C_WHITE)
  lcdDrawText(pctCentre, battTextY, pctStr,  MEDIUM+BOLD+CENTER+C_WHITE)
  lcdDrawText(usedX,     battTextY, usedStr, MEDIUM+BOLD+RIGHT+C_WHITE)
...
-- Preview box matches the size the flight screen draws model pictures at.
local function modelImgW() return FLIGHT.modelW or 192 end
local function modelImgH() return FLIGHT.modelH or 114 end
...
local function drawModelSelectScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  roundRect(zx, zy, SCREEN_W, BAR_H, 0, C_BLACK)
  lcdDrawText(zx+MI.titleX, zy+MI.titleY, STR.SELECT_MODEL_IMAGE, BIG+C_WHITE)
  ...
  lcdDrawText(zx+MI.infoX, zy+MI.modelY, "Model: " .. (widget.cachedModelName or ""), MEDIUM+C_LABEL)
  lcdDrawText(zx+MI.infoX, zy+MI.imageY, "Image: " .. currentImg, SMALL+C_ROW_TXT)
  ...
  if nImg > MI.visible then
    lcdDrawText(zx + MI.listX + MI.listW, zy + MI.imageY,
  ...
  roundRect(pvX - 2, pvY - 2, modelImgW() + 4, modelImgH() + 4, 4, C_SURFACE_HI)
  if previewBmp then
    lcdDrawBitmap(previewBmp, pvX, pvY)
  else
    roundRect(pvX, pvY, modelImgW(), modelImgH(), 0, C_CARD)
    lcdDrawText(pvX + modelImgW()/2, pvY + modelImgH()/2 - 8, "No preview", SMALL+CENTER+C_LABEL)
  end
  lcdDrawText(pvX + modelImgW()/2, pvY + modelImgH() + 8,
              hasPending and "Preview" or "Current", SMALL+CENTER+C_LABEL)

  if nImg == 0 then
    lcdDrawText(zx + MI.listX + MI.listW/2, zy+MI.emptyY1, "No images found in", MEDIUM+CENTER+C_LABEL)
    lcdDrawText(zx + MI.listX + MI.listW/2, zy+MI.emptyY2, "/Modelimage", MEDIUM+CENTER+C_LABEL)
  end
  ...
  if not hasPending then
    if drawButton(widget, tx, ty, clickEvent, zx + MI.backX, btnY, MI.btnW, MI.btnH, STR.BACK,
    ...
    if drawButton(widget, tx, ty, clickEvent, zx + MI.confirmX, btnY, MI.btnW, MI.btnH, STR.CONFIRM,
    ...
    if drawButton(widget, tx, ty, clickEvent, zx + MI.cancelX, btnY, MI.btnW, MI.btnH, STR.BACK,
...
-- Battery select screen: a two-column grid of battery cards, a header, and
-- the Back/Edit buttons.

local function drawSelectScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  roundRect(zx, zy, SCREEN_W, BAR_H, 0, C_BLACK)
  lcdDrawText(zx+SEL.titleX, zy+SEL.titleY, STR.SELECT_BATTERY, BIG+C_WHITE)
  ...
  if widget.recoveredBatteries then
    lcdDrawText(zx+SEL.noticeX, zy+SEL.noticeY, "Batteries restored from backup", SMALL+CENTER+C_ORANGE)
  elseif getTime() < safeNum(widget.saveFailedNotice, 0) then
    lcdDrawText(zx+SEL.noticeX, zy+SEL.noticeY, "SD write failed - flight not saved", SMALL+CENTER+C_RED)
  end

  local syncState, syncErr = battSyncStatus()
  if syncState == "error" then
    lcdDrawText(zx+SEL.syncX, zy+SEL.syncY, syncErr or "FC error", SMALL+RIGHT+C_RED)
  elseif syncState == "busy" then
    lcdDrawText(zx+SEL.syncX, zy+SEL.syncY, "FC sync...", SMALL+RIGHT+C_ORANGE)
  end
  ...
  if (now - initT) < 100 then
    ...
    lcdDrawText(zx+SEL.loadX, zy+SEL.loadY, STR.LOADING, MEDIUM+C_GREY)
    return clickEvent
  end
  ...
  local btnY, btnW, btnH = zy+SEL.btnY, SEL.btnW, SEL.btnH
  local backX = zx + SEL.backX
  local editX = zx + SEL.editX
  ...
  if drawButton(widget, tx, ty, (not hasModal) and clickEvent, backX, btnY, btnW, btnH,
                STR.BACK, {color=C_GREY, font=MEDIUM}) then
    setScreen(widget, "flight")
    clickEvent = false
  end
  ...
  for i = 1, maxIdx do
    ...
    if b then
      local isLeft = (i % 2 ~= 0)
      local cW, cH = SEL.cardW, SEL.cardH
      local cX = zx + (isLeft and SEL.colLX or SEL.colRX)
      local cY = zy + SEL.gridY + math.floor((i - 1) / 2) * SEL.rowStep
      ...
      lcdDrawText(cX+11, cY+SEL.nameDY, b.name or "Battery", MEDIUM+C_BLACK)
      lcdDrawText(cX+11, cY+SEL.fltDY, "(" .. safeNum(b.flights, 0) .. " flt)", SMALL+C_GREY)
    end
  end

  if widget.pendingBatteryIndex then
    local popW, popH = SEL.popW, SEL.popH
    local popX = zx + math.floor((SCREEN_W - popW) / 2)
    local popY = zy + math.floor((SCREEN_H - popH) / 2)
    ...
    roundRect(popX, popY, popW, popH, 6, C_SURFACE)
    roundRect(popX, popY, popW, MODAL_H, 6, C_BLACK)
    lcdDrawText(popX + popW/2, popY + textCenterY(MODAL_H, MEDIUM), "CONFIRM SELECTION", MEDIUM+CENTER+C_WHITE)
    ...
    lcdDrawText(popX + popW/2, popY + MD.line2, infoStr, MEDIUM+CENTER+C_LABEL)

    local mBtnW, mBtnH = 130, MD.btnH
    local mBtnY = popY + popH - mBtnH - 14
    local cnfX = popX + 30
    ...
    local focus1 = (widget.popupFocus == 1)
    if drawButton(widget, tx, ty, clickEvent, cnfX, mBtnY, mBtnW, mBtnH, STR.CONFIRM,
    ...
    local focus2 = (widget.popupFocus == 2)
    if drawButton(widget, tx, ty, clickEvent, canX, mBtnY, mBtnW, mBtnH, STR.BACK,
...
-- SUMMARY SCREEN
-- ===========================================================================
-- Post-flight summary: a two-column grid of label/value cards.
...
local function drawSummaryScreen(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt)
  ...
  roundRect(zx,zy,SCREEN_W,40,0,C_BLACK)
  lcdDrawText(zx+SUM.titleX,zy+SUM.titleY,STR.SUMMARY,BIG+C_WHITE)

  local now = getTime()
  local sumT = safeNum(widget.summaryStartTime, now)
  if now - sumT > 30000 then setScreen(widget, "flight") end

  local bX,bY,bW,bH = zx+SUM.btnX,zy+SUM.btnY,SUM.btnW,SUM.btnH
  if drawButton(widget, tx, ty, clickEvent, bX, bY, bW, bH, STR.BACK,
  ...
  local rH,rW = SUM.rowStep, SUM.cardW
  for idx = 1, #SUMMARY_LABELS do
    local col = (idx%2==1) and SUM.colLX or SUM.colRX
    local rY  = zy+SUM.gridY+math.floor((idx-1)/2)*rH
    roundRect(zx+col,rY,rW,SUM.cardH,4,C_ROW)
    lcdDrawFilledRectangle(zx+col,rY,SUM.edgeW,SUM.cardH,C_ROW_EDGE)
    lcdDrawText(zx+col+SUM.labelDX,rY+SUM.labelDY, SUMMARY_LABELS[idx], SMALL+C_LABEL)
    lcdDrawText(zx+col+SUM.labelDX,rY+SUM.valueDY, cache[idx] or "-",   MEDIUM+C_ROW_TXT)
  end
  return clickEvent
end
...
local function create(zone, opts)
  opts = opts or {}
  -- Pick the layout profile before anything else: every position used below
  -- comes from the tables it fills.
  applyLayout(zone and zone.w, zone and zone.h)
  ...
  -- Each resolution needs its own artwork, since the cards are drawn in it.
  -- The 800x480 file is looked for first when running at that size, falling
  -- back to the plain name so an existing single-resolution install still
  -- works unchanged.
  -- Both capitalisations are tried: SD cards are usually case-insensitive,
  -- but the EdgeTX simulator on Linux is not, so a file saved as
  -- "Background.png" was invisible to a lowercase-only lookup.
  local bgDir = "/WIDGETS/"..name.."/"
  local bgStems = (SCREEN_W >= 800)
        and { "background800", "Background800", "background", "Background" }
        or  { "background", "Background" }
  local bgBmp
  for _, stem in ipairs(bgStems) do
    for _, ext in ipairs({ ".png", ".bmp", ".jpg" }) do
      bgBmp = bgBmp or safeOpenBitmap(bgDir .. stem .. ext)
    end
  end
  ...
  local widget = {
    ...
    vals={volt=-1,cell=-1,curr=-1,minCell=-1,maxCurr=-1,temp=-1,bec=-1,rqly=-2,thr=-1,nameStr="",nameW=0,usedStr="",usedW=0,pctStr="",pctW=0,timer=-99999,txVolt=-1,clockMin=-1,mode=-999,rpm=-999,bank=-999},
...
local function pidTuneHelpers()
  ...
  PIDTUNE_HELPERS = PIDTUNE_HELPERS or {
    ...
    -- Layout and screen size, so the tuning screen adapts to the same
    -- resolutions as the rest of the widget instead of assuming 480x320.
    RIGHT=RIGHT, PT=PT,
  }
  -- PT is a table reference so it tracks applyLayout automatically, but the
  -- screen size is copied by value -- refresh it in case the profile changed
  -- since the helpers were first built.
  PIDTUNE_HELPERS.SCREEN_W = SCREEN_W
  PIDTUNE_HELPERS.SCREEN_H = SCREEN_H
...
local function draw(widget, keyEvt)
  ...
  local zw = safeNum(widget.zone and widget.zone.w, SCREEN_W)
  local zh = safeNum(widget.zone and widget.zone.h, SCREEN_H)
  local zx = safeNum(widget.zone and widget.zone.x, 0)
  local zy = safeNum(widget.zone and widget.zone.y, 0)

  -- The zone can change without create() being called again (layout edits),
  -- so re-resolve the profile if it no longer matches.
  if zw >= 800 and zh >= 480 and SCREEN_W ~= 800 then applyLayout(zw, zh) end
  if zw < 800 and SCREEN_W ~= 480 then applyLayout(zw, zh) end

  if zw < SCREEN_W or zh < SCREEN_H then
