...
function M.draw(widget, opts, zx, zy, tx, ty, clickEvent, keyEvt, helpers)
  ...
  -- Header
  roundRect(zx, zy, 480, 36, 0, C_BLACK)
  lcdDrawText(zx + 10, zy + 8, "Tuning", MEDIUM + C_WHITE)

  local profileTxt = ""
  if widget.fcProfile ~= nil then
    profileTxt = string.format("P%d / R%d", (widget.fcProfile or 0) + 1, (widget.fcRateProfile or 0) + 1)
  end
  lcdDrawText(zx + 320, zy + 11, widget.fcProfile ~= nil and profileTxt or "", SMALL + C_YELLOW)

  local bX, bY, bW, bH = zx + 400, zy + 4, 70, 28
  ...
  roundRect(zx, zy + 36, 480, 284, 0, C_SURFACE)

  -- Tabs
  local tabY, tabH = zy + 40, 28
  local pidTabX, ratesTabX, govTabX, tabW = zx + 10, zx + 100, zx + 190, 85
  ...
  lcdDrawText(zx + 290, tabY + 6, t.status or "", SMALL + C_LABEL)

  -- Waiting for FC Check
  if type(t.data) ~= "table" or next(t.data) == nil then
    lcdDrawText(zx + 20, zy + 100, "Waiting for flight controller...", SMALL + C_LABEL)
    lcdDrawText(zx + 20, zy + 120, "Status: " .. tostring(t.status or "Offline"), SMALL + C_LABEL)
    if tab == "gov" and not widget.govTuning then
      lcdDrawText(zx + 20, zy + 140, "Error: MSP/mspGovernorProfile script not found.", SMALL + C_RED)
    end
    return nil
  end
  ...
  if tab == "rates" then
    ...
    if typeName ~= "ROTORFL" then
      lcdDrawText(zx + 20, zy + 100, "Unsupported rate type: " .. tostring(typeName or "Unknown"), SMALL + L_ORANGE)
      lcdDrawText(zx + 20, zy + 120, "This screen only supports ROTORFL rates.", SMALL + C_LABEL)
      return nil
    end
  end
  ...
  if tab == "gov" then
    ...
    local colW = 150
    local itemH = 36
    local startX, startY = zx + 15, zy + 78
    ...
    local sX, sY, sW, sH = zx + 20, zy + 280, 120, 36
  ...
  local colW  = 113
  local startX, startY = zx + 1, zy + 78
